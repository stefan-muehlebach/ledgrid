...
func NewCanvas(size image.Point) *Canvas {
	...
	c.Mask = image.NewUniform(color.Alpha{0xff})
...
// Zeichnet alle (sichtbaren) Objekte dieses Canvas'es neu. Die Reihenfolge
// des Neuaufbaus ist dabei ObjList von vorne (Front) nach hinten (Back).
...
// Leider gibt es (aufgrund historischer Umstaende) drei Arten, wie Positionen
// verwaltet werden koennen: als geom.Point (mit X und Y als float64), als
// image.Point (mit X und Y als int) und schliesslich als fixed.Point26_6
// (mit X und Y als fixed.Int26_6).
...
// Fuer groessen gibt es dagegen nur einen Typ, der Breite und Hoehe als
// geom.Point speichert.
...
// Fuer die Farbgebung gibt es zwei Embeddable: ColorEmbed fuer einfarbige
// Objekte oder fuer den Rahmen/Rand mehrfarbiger Objekte.
...
// Mit diesem Embeddable wird werden Rahmen UND Fuellfarbe verwaltet. Ein
// Objekt mit eigener Farbe fuer Rahmen und Fuellung bindet nur dieses eine
// Embeddable ein.
