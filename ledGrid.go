...
const (
	framesPerSecond = 50
	frameRefresh    = time.Second / framesPerSecond
	frameRefreshMs  = 1000 / framesPerSecond
	frameRefreshSec = float64(frameRefreshMs) / 1000.0
)

// Alles, was sich irgendwie animieren und auf dem LedGrid darstellen laesst,
// muss das Interface VisualEffect (TO DO: ev. waere Animatable und/oder
// Drawable besser)
type Animatable interface {
	// Animiert das Objekt, wobei t ein Point-in-Time in Sekunden und
	// Bruchteilen ist. Falls Update false retourniert, ist das Objekt mit
	// der Animation fertig, darf nicht mehr gezeichnet werden und kann vom
	// aufrufenden Programm geloescht werden.
