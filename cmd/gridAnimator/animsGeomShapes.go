...
func RectanglesJourney(ctx context.Context, c *ledgrid.Canvas) {
	...
	rectPos1 := geom.Point{w - 4.0, h / 2.0}
	rectPos2 := geom.Point{w / 2.0, h / 2.0}
	rectPos3 := geom.Point{4.0, h / 2.0}
	rectSize := geom.Point{7.0, 3.0}

	rect := ledgrid.NewRectangle(rectPos1, rectSize, colors.GreenYellow.Alpha(0.0))

	dotList[0] = ledgrid.NewDot(geom.Point{-2.0, 0.0}, colors.LightBlue)
	dotList[1] = ledgrid.NewDot(geom.Point{-2.0, 0.0}, colors.LightBlue.Dark(0.2))
	dotList[2] = ledgrid.NewDot(geom.Point{-2.0, 0.0}, colors.LightBlue.Dark(0.4))
	...
	flyToEnd := ledgrid.NewPositionAnim(rect, rectPos3, 1000*time.Millisecond)
	flyToEnd.Curve = ledgrid.AnimationEaseOut
	flyToStart := ledgrid.NewPositionAnim(rect, rectPos1, time.Millisecond)
	flyToStart.Curve = ledgrid.AnimationLinear
	...
	for i := range 3 {
		...
		animList[i].Val1 = func() geom.Point {
			posList[i] = geom.Point{-2.0, float64(rand.IntN(10))}
			return posList[i]
		}
		animList[i].Val2 = func() geom.Point {
			return posList[i].AddXY(w+5.0, 0.0)
		}
	...
	seq := ledgrid.NewSequence(fadeIn, flyToCenter, tl, flyToEnd, fadeOut, flyToStart)
	seq.RepeatCount = ledgrid.AnimationRepeatForever
